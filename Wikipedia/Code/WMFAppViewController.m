...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            NSString *latString = activity.userInfo[@"WMFPlacesLatitude"];
            NSString *lonString = activity.userInfo[@"WMFPlacesLongitude"];
            
            if (latString && lonString) {
                double latitude = [latString doubleValue];
                double longitude = [lonString doubleValue];
                [[self placesViewController] showLocationWithLatitude:latitude longitude:longitude];
            } else {
                // Existing handling for article URLs
